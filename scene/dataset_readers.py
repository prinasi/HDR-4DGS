...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", time_duration=None, frame_ratio=1, dataloader=False):
    ...
    def frame_read_fn(idx_frame):
        ...
        if not frame["file_path"].endswith((".png", "jpeg")):
        ...
        return CameraInfo(uid=idx, R=R, T=T, FovY=fovy, FovX=fovx, image=image, depth=depth, image_hdr=image_hdr, exposure=exposures,
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png", num_pts=50_000, time_duration=None, num_extra_pts=0, frame_ratio=1, dataloader=False):
    ...
    test_cam_infos = readCamerasFromTransforms(path, "transforms_test.json", white_background, extension, time_duration=time_duration, frame_ratio=frame_ratio, dataloader=dataloader)
    ...
    if not os.path.exists(ply_path):
        ...
        # xyz = np.random.random((num_pts, 3)) * 10.0 - 5.0  # for deer only
        xyz = np.random.random((num_pts, 3)) * 2.6 - 1.3  # 2.6
        shs = np.random.random((num_pts, 3)) / 255.0
        pcd = BasicPointCloud(points=xyz, colors=SH2RGB(shs), normals=np.zeros((num_pts, 3)))
        # storePly(ply_dst_path, xyz, SH2RGB(shs) * 255)
    else:
        print("Found existing point cloud!")
        pcd = fetchPly(ply_path)
